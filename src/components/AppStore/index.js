...
class AppStore extends Component {
  state = {activeId: tabsList[0].tabId, textValue: ''}
  ...
  filterApp = () => {
    ...
    const filteredApp = appsList.filter(
      eachItem => eachItem.category === activeId,
    )
    return filteredApp
  }
  ...
  render() {
    const {activeId, textValue} = this.state
    const filterAppList = this.filterApp()
    const searchItemList = filterAppList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(textValue.toLowerCase()),
    )
    return (
          ...
          <ul className="tab-container">
            {tabsList.map(eachList => (
              <TabItem
                key={eachList.tabId}
                tabDetails={eachList}
                displayId={this.displayTabId}
                isActive={activeId === eachList.tabId}
              />
            ))}
          </ul>
          <ul className="app-list-container">
            {searchItemList.map(eachApp => (
